...
class ADVAIPBL_CLI extends WP_CLI_Command
{
    ...
    public function session($args, $assoc_args)
    {
        ...
        if (! in_array($subcommand, ['list', 'terminate'], true)) {
            WP_CLI::error("Invalid subcommand. Use 'list' or 'terminate'.");
        ...
        if ('list' === $subcommand) {
            ...
            if ($role_filter) {
                $sessions = array_filter($sessions, function ($session) use ($role_filter) {
                    return in_array($role_filter, explode(', ', $session['role']), true);
                });
            }
            if (empty($sessions)) {
                WP_CLI::line('No active user sessions found.');
            ...
            if (isset($assoc_args['user-id'])) {
                ...
                if ($user_id_to_logout > 0) {
                    ...
                    $log_message = sprintf(/* translators: %s is a placeholder */ __('Sessions for user ID %d terminated via WP-CLI.', 'advanced-ip-blocker'), $user_id_to_logout);
                }
                ...
                if ($user) {
                    ...
                    $log_message = sprintf(/* translators: %s is a placeholder */ __('Sessions for user \'%s\' terminated via WP-CLI.', 'advanced-ip-blocker'), $assoc_args['user-login']);
                }
            } elseif (isset($assoc_args['role'])) {
                $user_ids_to_logout = get_users(['role' => $assoc_args['role'], 'fields' => 'ID']);

                $log_message = sprintf(/* translators: %s is a placeholder */ __('All sessions for role \'%s\' terminated via WP-CLI.', 'advanced-ip-blocker'), $assoc_args['role']);
    ...
    {
        $subcommand = array_shift($args);
        if (! in_array($subcommand, [ 'add', 'remove', 'list' ], true)) {
            WP_CLI::error('Invalid subcommand. Use: add, remove, or list.');
        }

        if (in_array($subcommand, [ 'add', 'remove' ], true)) {
            if (empty($args[0])) {
                WP_CLI::error('Please provide an IP or range.');
        ...
        switch ($subcommand) {
            ...
            case 'remove':
                if (! array_key_exists($ip_or_range, $list)) {
                    WP_CLI::warning("Entry '{$ip_or_range}' was not in the whitelist.");
                ...
                $this->log_event_autonomo(sprintf(/* translators: %s is a placeholder */ __('Entry %s removed from whitelist via WP-CLI.', 'advanced-ip-blocker'), $ip_or_range), 'info', $ip_or_range);
                WP_CLI::success("Entry '{$ip_or_range}' removed from whitelist.");
                break;
            case 'list':
                if (empty($list)) {
                    WP_CLI::line('The whitelist is empty.');
                ...
                foreach ($list as $ip_addr => $data) {
                    ...
                    if (is_array($data) && isset($data['timestamp'])) {
                        ...
                        $timestamp = $data;
                        $detail = 'Migrated from old format';
                    }
     ...
     *
     * [<provider>]
     * : The name of the API provider (e.g., ip-api.com).
     *
     * [<api_key>]
     * : The API key (required for set_key).
     *
     * ## EXAMPLES
     *
     *     # List all providers and their statuses
     *     $ wp advaipbl provider list
     *
     *     # Set the active provider
     *     $ wp advaipbl provider set ipinfo.io
     *
     *     # Configure an API key for a provider
     *     $ wp advaipbl provider set_key ipinfo.io YOUR_API_KEY
     *
     * @subcommand provider
     */
    public function provider($args, $assoc_args)
    {
        ...
        switch ($subcommand) {
            case 'list': $this->provider_list($providers_map);
                break;
            case 'set': $this->provider_set($args, $providers_map);
                break;
            case 'set_key': $this->provider_set_key($args, $providers_map);
                break;
            case 'remove_key': $this->provider_remove_key($args, $providers_map);
                break;
        }
    }
    ...
    private function provider_list($providers_map)
    {
        ...
        foreach ($providers_map as $provider => $key_name) {
            ...
            if ($key_name !== null) {
                $api_key_status = !empty($settings[$key_name]) ? 'Set' : 'Not Set';
            }
    ...
    {
        if (empty($args[0])) {
            WP_CLI::error("Please specify a provider. Usage: wp advaipbl provider set <provider-name>");

            return;
        }
        $new_provider = $args[0];
        if (!array_key_exists($new_provider, $providers_map)) {
            WP_CLI::error("Invalid provider. Available providers are: " . implode(', ', array_keys($providers_map)));
    ...
    {
        if (count($args) < 2) {
            WP_CLI::error("Usage: wp advaipbl provider set_key <provider-name> <api-key>");

            return;
        }
        list($provider, $api_key) = $args;
        if (!isset($providers_map[$provider]) || $providers_map[$provider] === null) {
            WP_CLI::error("'{$provider}' is not a valid provider or does not support API keys.");
    ...
    {
        if (empty($args[0])) {
            WP_CLI::error("Usage: wp advaipbl provider remove_key <provider-name>");
     ...
     *
     * [<country_code>]
     * : The 2-letter country code (e.g., US, DE). Required for 'add' and 'remove'.
     *
     * ## EXAMPLES
     *
     *     # List blocked countries
     *     $ wp advaipbl geoblock list
     *
     *     # Add a country to the block list
     *     $ wp advaipbl geoblock add US
     *
     * @subcommand geoblock
     */
    public function geoblock($args, $assoc_args)
    {
        ...
        if (! is_array($blocked_countries_raw)) {
            $blocked_countries = empty($blocked_countries_raw) ? [] : [ (string) $blocked_countries_raw ];
        } else {
            $blocked_countries = $blocked_countries_raw;
        }
        switch ($subcommand) {
            case 'add':
                if (count($args) < 1) {
                    WP_CLI::error('Please provide a 2-letter country code. Usage: wp advaipbl geoblock add <country_code>');

                    return;
                }
                $country_code = strtoupper($args[0]);
                if (strlen($country_code) !== 2) {
                    WP_CLI::error('Country code must be 2 letters (e.g., US, DE, CN).');

                    return;
                }
                if (in_array($country_code, $blocked_countries, true)) {
                    WP_CLI::warning("Country {$country_code} is already in the block list.");
                ...
                $this->log_event_autonomo(sprintf(/* translators: %s is a placeholder */ __('Country %s added to geoblock list via WP-CLI.', 'advanced-ip-blocker'), strtoupper($country_code)), 'warning');
                WP_CLI::success("Country {$country_code} added to the block list.");
                break;
            case 'remove':
                if (count($args) < 1) {
                    WP_CLI::error('Please provide a country code to remove.');
                ...
                if (false === $key) {
                    WP_CLI::warning("Country {$country_code} was not found in the block list.");
                ...
                $this->log_event_autonomo(sprintf(/* translators: %s is a placeholder */ __('Country %s removed from geoblock list via WP-CLI.', 'advanced-ip-blocker'), strtoupper($country_code)), 'info');
                WP_CLI::success("Country {$country_code} removed from the block list.");
                break;
            case 'list':
                if (empty($blocked_countries)) {
                    WP_CLI::line('The country block list is empty.');

                    return;
                }
                WP_CLI::line('Blocked country codes:');
                foreach ($blocked_countries as $code) {
                    WP_CLI::line("- {$code}");
                }
                break;
        }
     ...
     *
     * [--type=<type>]
     * : Filter by log type (e.g., general, geoblock, 404).
     * ---
     * default: all
     * ---
     *
     * [--count=<count>]
     * : Number of items to retrieve (for 'list').
     * ---
     * default: 20
     * ---
     *
     ...
     *
     * [--force]
     * : Skip confirmation prompt when clearing logs.
     *
     * ## EXAMPLES
     *
     *     # List the last 20 logs
     *     $ wp advaipbl log list
     *
     *     # Clear all logs without confirmation
     *     $ wp advaipbl log clear --force
     *
     * @subcommand log
     */
    public function log($args, $assoc_args)
    {
        $subcommand = array_shift($args);
        switch ($subcommand) {
            case 'list': $this->log_list($args, $assoc_args);
                break;
            case 'clear': $this->log_clear($args, $assoc_args);
                break;
    ...
    private function log_list($args, $assoc_args)
    {
        global $wpdb;
        $table_name = $wpdb->prefix . 'advaipbl_logs';
        $type = $assoc_args['type'] ?? 'general';
        $count = $assoc_args['count'] ?? 20;
        $where_clause = '1=1';
        if ('all' !== $type) {
            $where_clause = $wpdb->prepare('log_type = %s', $type);
        }
        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter
        ...
        if (empty($logs)) {
            WP_CLI::line('No log entries found for the specified type.');
    ...
    private function log_clear($args, $assoc_args)
    {
        ...
        if (! isset($assoc_args['force'])) {
            $confirm_message = ('all' === $type) ? 'Are you sure you want to clear ALL logs? This action cannot be undone.' : "Are you sure you want to clear all '{$type}' logs?";
            WP_CLI::confirm($confirm_message);
        }
        global $wpdb;
        $table_name = $wpdb->prefix . 'advaipbl_logs';
        $this->log_event_autonomo("WP-CLI: Clearing logs of type '{$type}'.", 'warning');
        if ('all' !== $type) {
     ...
     *     $ wp advaipbl config get
     *
     *     # Get a specific config value
     *     $ wp advaipbl config get enable_geoblocking
     *
     *     # Set a config value
     *     $ wp advaipbl config set enable_geoblocking 1
     *
     * @subcommand config
     */
    ...
    {
        if (empty($args[0])) {
            WP_CLI::error("Please specify a subcommand: 'get' or 'set'.");
        ...
        switch ($subcommand) {
            case 'get':
                ...
                if ($key) {
                    if (isset($options[ $key ])) {
                        ...
                        WP_CLI::error("Option '{$key}' not found in advaipbl_settings.");
                    }
                } else {
                    $display_items = [];
                    foreach ($options as $option_key => $option_value) {
                        $display_items[] = [ 'key' => $option_key, 'value' => is_array($option_value) ? wp_json_encode($option_value) : $option_value, ];
                    }
            ...
            case 'set':
                if (count($args) < 2) {
                    WP_CLI::error('Usage: wp advaipbl config set <key> <value>');
                ...
                $options = get_option(ADVAIPBL_Main::OPTION_SETTINGS, []);
                list($key, $value) = $args;
                $old_sanitized_value = $options[$key] ?? null;
                $input_to_sanitize = array_merge($options, [$key => $value]);
                $sanitized_options = $main_instance->settings_manager->sanitize_settings($input_to_sanitize);
                if (!isset($sanitized_options[$key])) {
                    WP_CLI::error("Invalid key '{$key}'.");

                    return;
                }
                $new_sanitized_value = $sanitized_options[$key];
                if ($old_sanitized_value === $new_sanitized_value && $value !== (string)$old_sanitized_value) {
                    WP_CLI::error("Invalid value '{$value}' provided for key '{$key}'. Please provide a valid value.");
                ...
                $this->log_event_autonomo(sprintf(/* translators: %s is a placeholder */ __('Option \'%1$s\' updated to \'%2$s\' via WP-CLI.', 'advanced-ip-blocker'), $key, $log_value), 'info');
     ...
     *
     * [<rule>]
     * : The rule to add or remove. Required for 'add' and 'remove'.
     *
     * [--from-file=<file>]
     * : Load rules from a file (used with 'add').
     *
     * ## EXAMPLES
     *
     *     # List all WAF rules
     *     $ wp advaipbl waf list
     *
     *     # Add a rule from a file
     *     $ wp advaipbl waf add --from-file=rules.txt
     *
     * @subcommand waf
     */
        ...
        switch ($subcommand) {
            case 'add':
                ...
                if ($file_path) {
                    ...
                    WP_CLI::error("Please provide a rule to add, or use the --from-file=<file> flag.");

                    return;
                }
                $added_count = 0;
                foreach ($new_rules_to_add as $new_rule) {
                    if (! in_array($new_rule, $rules, true)) {
                        $rules[] = $new_rule;
                        $added_count++;
                    }
                }
                if ($added_count > 0) {
                    ...
                    $this->log_event_autonomo(sprintf(/* translators: %s is a placeholder */ __('%d WAF rule(s) added via WP-CLI.', 'advanced-ip-blocker'), $added_count), 'info');
            ...
            case 'remove':
                if (empty($args[0])) {
                    WP_CLI::error('Please provide a rule to remove.');
                ...
                if ($key_to_remove === false) {
                    WP_CLI::warning("Rule not found: '{$rule_to_remove}'");
                ...
                $this->log_event_autonomo(sprintf(/* translators: %s is a placeholder */ __('WAF rule "%s" removed via WP-CLI.', 'advanced-ip-blocker'), $rule_to_remove), 'info');
                WP_CLI::success("Rule removed successfully.");
                break;
            case 'list':
                if (empty($rules)) {
                    WP_CLI::line('The WAF ruleset is empty.');
     ...
     * Private function called by the public 'waf' command.
     */
    private function waf_exclude($args, $assoc_args)
    {
        ...
        switch ($action) {
            ...
            case 'add':
                if (empty($args[0])) {
                    WP_CLI::error('Please provide a URL fragment to add.');
            ...
            case 'remove':
                if (empty($args[0])) {
                    WP_CLI::error('Please provide a URL fragment to remove.');
     ...
     *
     * [<asn>]
     * : The ASN to block (e.g., AS12345). Required for 'add' and 'remove'.
     *
     * ## EXAMPLES
     *
     *     # Block a specific ASN
     *     $ wp advaipbl asn add AS12345
     *
     *     # List all blocked ASNs
     *     $ wp advaipbl asn list
     *
     * @subcommand asn
     */
    public function asn($args, $assoc_args)
    {
        ...
        if (!is_array($blocked_asns)) {
            $blocked_asns = [];
        }
        switch ($subcommand) {
            case 'add':
                if (empty($args[0])) {
                    WP_CLI::error('Please provide an ASN to add (e.g., AS12345).');

                    return;
                }
                $asn_to_add = strtoupper(trim($args[0]));
                if (! preg_match('/^AS\d+$/i', $asn_to_add)) {
                    WP_CLI::error("Invalid ASN format. Please use the format 'AS' followed by numbers (e.g., AS15169).");

                    return;
                }
                if (in_array($asn_to_add, $blocked_asns, true)) {
                    WP_CLI::warning("ASN {$asn_to_add} is already in the block list.");
            ...
            case 'remove':
                if (empty($args[0])) {
                    WP_CLI::error('Please provide an ASN to remove.');
                ...
                if (false === $key) {
                    WP_CLI::warning("ASN {$asn_to_remove} was not found in the block list.");
            ...
            case 'list':
                if (empty($blocked_asns)) {
                    WP_CLI::line('The ASN block list is empty.');
     ...
     *     $ wp advaipbl xmlrpc_mode disabled
     *
     *     # Enable smart protection mode
     *     $ wp advaipbl xmlrpc_mode smart
     */
    public function xmlrpc_mode($args, $assoc_args)
    {
        if (empty($args[0])) {
            WP_CLI::error("Please specify a mode. Usage: wp advaipbl xmlrpc-mode <enabled|smart|disabled>");
        ...
        if (! in_array($mode, $valid_modes, true)) {
            WP_CLI::error("Invalid mode '{$mode}'. Please use 'enabled', 'smart', or 'disabled'.");
    ...
    private function score_get($args, $assoc_args)
    {
        ...
        foreach ($history as $event) {
            ...
            if (!empty($event['details'])) {
                if (isset($event['details']['rule'])) {
                    $details_str = 'Rule: ' . $event['details']['rule'];
                } elseif (isset($event['details']['url'])) {
                    $details_str = 'URL: ' . $event['details']['url'];
                } elseif (isset($event['details']['uri'])) {
                    $details_str = 'URI: ' . $event['details']['uri'];
                } elseif (isset($event['details']['username'])) {
                    $details_str = 'User: ' . $event['details']['username'];
                }
            }
    ...
    public function tfa_status($args, $assoc_args)
    {
        ...
        if (empty($args[0])) {
            WP_CLI::error("Please specify a user login, email, or ID.");
    ...
    public function geo_challenge($args, $assoc_args)
    {
        ...
        switch ($subcommand) {
            case 'add':
                if (count($args) < 1) {
                    WP_CLI::error('Please provide a 2-letter country code. Usage: wp advaipbl geo-challenge add <country_code>');
                ...
                if (in_array($country_code, $challenged_countries, true)) {
                    WP_CLI::warning("Country {$country_code} is already in the challenge list.");
            ...
            case 'remove':
                ...
                if (false === $key) {
                    WP_CLI::warning("Country {$country_code} was not found in the challenge list.");
    ...
    public function asn_whitelist($args, $assoc_args)
    {
        ...
        switch ($subcommand) {
            case 'add':
                ...
                if (in_array($asn_to_add, $whitelisted_asns, true)) {
                    WP_CLI::warning("ASN {$asn_to_add} is already in the whitelist.");
            ...
            case 'remove':
                ...
                if (false === $key) {
                    WP_CLI::warning("ASN {$asn_to_remove} was not found in the whitelist.");
        ...
        switch ($subcommand) {
            ...
            case 'whitelist':
                ...
                if ($subcommand === 'delete') {
                    ...
                    $main_instance->fingerprint_manager->delete_signature($hash);
