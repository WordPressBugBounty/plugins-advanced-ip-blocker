...
class ADVAIPBL_Site_Scanner
{
    ...
    /**
     * Performs a complete scan of the local environment.
    ...
    /**
     * Scans installed plugins against the remote AIB API.
     * @return array
     */
    ...
    /**
     * Checks the server IP reputation against configured blocklists.
    ...
    public function check_server_reputation()
    {
        ...
        if (! empty($this->plugin->options['enable_abuseipdb']) && ! empty($this->plugin->options['abuseipdb_api_key'])) {
            ...
            if ($check && isset($check['score'])) {
                if ($check['score'] > 0) {
                    $abuse_status = ($check['score'] > 20) ? 'listed' : 'warning';
                    if ($check['score'] > 20) {
                        $results['status'] = 'blacklisted';
                    }
                } else {
    ...
    public function run_full_scan_and_email($to_email, $is_manual = false)
    {
        ...
        if (! $is_manual) {
            ...
            if ($has_vuln_or_critical) {
                ...
                if ($local_scan['updates']['status'] !== 'good' && $local_scan['updates']['status'] !== 'skipped') {
                    $has_any_issue = true;
                }
                if ($local_scan['php']['status'] !== 'good' && $local_scan['php']['status'] !== 'skipped') {
                    $has_any_issue = true;
                }
                if ($local_scan['wordpress']['status'] !== 'good' && $local_scan['wordpress']['status'] !== 'skipped') {
                    $has_any_issue = true;
                }
                if ($local_scan['ssl']['status'] !== 'good' && $local_scan['ssl']['status'] !== 'skipped') {
                    $has_any_issue = true;
                }
                if ($local_scan['debug_mode']['status'] !== 'good' && $local_scan['debug_mode']['status'] !== 'skipped') {
                    $has_any_issue = true;
                }
            }
