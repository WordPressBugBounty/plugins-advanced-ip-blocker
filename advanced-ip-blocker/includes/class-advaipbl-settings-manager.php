...
class ADVAIPBL_Settings_Manager
{
    ...
    public function register_settings()
    {
        ...
        add_settings_field(
            ...
            $page,
            'advaipbl_geolocation_section',
            ['class' => 'advaipbl-geolocation-api-option']
        );
        ...
        add_settings_field(
            ...
            [
                ...
                'description' => sprintf(/* translators: %s is a placeholder */ __('Manage Honeypot URLs in the <a href="%s">Blocking Rules</a> tab.', 'advanced-ip-blocker'), admin_url('admin.php?page=advaipbl_settings_page&tab=rules&sub-tab=honeypot')),
        ...
        add_settings_field(
            ...
            [
                ...
                'help_url' => 'https://advaipbl.com/verify-ai-bots-cidr-guide/'
            ]
        );

        add_settings_field(
            'advaipbl_enable_monitoring_bot_verification',
            __('Verify Monitoring Bots (IP List)', 'advanced-ip-blocker'),
            [$this, 'switch_field_callback'],
            $page,
            'advaipbl_user_agent_settings_section',
            [
                'name' => 'enable_monitoring_bot_verification',
                'label' => __('Enable IP verification for Monitoring Bots (e.g., Uptime Robot, Pingdom).', 'advanced-ip-blocker'),
                'description' => __('Downloads official IP lists from monitoring services to allow legitimate pings and block impersonators.', 'advanced-ip-blocker'),
                'help_url' => 'https://advaipbl.com/verify-monitoring-bots-ip-list-complete-guide/'
            ]
        );
        ...
        add_settings_field('advaipbl_enable_manual_asn', __('Manual ASN Protection', 'advanced-ip-blocker'), [$this, 'switch_field_callback'], $page, 'advaipbl_asn_protection_section', ['name' => 'enable_manual_asn', 'label' => __('Enable blocking based on your custom Manual ASN Blocklist.', 'advanced-ip-blocker'), 'description' => sprintf(/* translators: %s is a placeholder */ __('Manage your custom list in the <a href="%s">Blocking Rules</a> tab.', 'advanced-ip-blocker'), admin_url('admin.php?page=advaipbl_settings_page&tab=rules&sub-tab=asn_blocking'))]);
        ...
        add_settings_field(
            ...
            [
                ...
                'description' => sprintf(/* translators: %s is a placeholder */ __('Manage WAF rules in the <a href="%s">Blocking Rules</a> tab.', 'advanced-ip-blocker'), admin_url('admin.php?page=advaipbl_settings_page&tab=rules&sub-tab=waf')),
        ...
        add_settings_field('advaipbl_turnstile_site_key', __('Turnstile Site Key', 'advanced-ip-blocker'), [$this, 'password_field_callback'], $page, 'advaipbl_captcha_integrations_section', [
            ...
            'default' => '',
            'description' => sprintf(
                /* translators: %s is a placeholder */ __('You can get your Turnstile keys from the <a href="%s" target="_blank" rel="noopener noreferrer">Cloudflare Dashboard</a>.', 'advanced-ip-blocker'),
        ...
        add_settings_field('advaipbl_hcaptcha_site_key', __('hCaptcha Site Key', 'advanced-ip-blocker'), [$this, 'password_field_callback'], $page, 'advaipbl_captcha_integrations_section', [
            ...
            'default' => '',
            'description' => sprintf(
                /* translators: %s is a placeholder */ __('You can get your hCaptcha keys from the <a href="%s" target="_blank" rel="noopener noreferrer">hCaptcha Dashboard</a>.', 'advanced-ip-blocker'),
        ...
        add_settings_field(
            ...
            [
                ...
                'description' => sprintf(/* translators: %s is a placeholder */ __('When enabled, users will see the 2FA setup section in their  <a href="%s">Profile</a>.', 'advanced-ip-blocker'), admin_url('profile.php'))
            ]
        );
        ...
        add_settings_field(
            ...
            [
                ...
                // phpcs:ignore PluginCheck.CodeAnalysis.Offloading.OffloadedContent
                'description' => sprintf(/* translators: %s is a placeholder */ __('Create a token with <strong>Zone > Firewall > Edit</strong> permissions. <a href="%s" target="_blank">Get Token</a>', 'advanced-ip-blocker'), 'https://dash.cloudflare.com/profile/api-tokens'),
        ...
        add_settings_field(
            'advaipbl_fim_scan_frequency',
            __('Scan Frequency', 'advanced-ip-blocker'),
            [$this, 'select_field_callback'],
            $page,
            'advaipbl_internal_security_section',
            [
                'name' => 'fim_scan_frequency',
                'options' => [
                    'daily'             => __('Daily (Recommended)', 'advanced-ip-blocker'),
                    'twicedaily'        => __('Twice a day (Every 12 hours)', 'advanced-ip-blocker'),
                    'advaipbl_6_hours'  => __('High Security (Every 6 hours - Warning: Consumes more resources)', 'advanced-ip-blocker')
                ]
            ]
        );
        ...
        add_settings_field(
            ...
            [$this, 'fim_manual_scan_button_callback'],
            $page,
            'advaipbl_internal_security_section'
        );
    }
    ...
    public function sanitize_settings($input)
    {
        ...
        $checkbox_fields = [
            ...
            'enable_bot_verification', 'enable_ai_bot_verification', 'enable_monitoring_bot_verification',
        ...
        $text_fields = [
            ...
            'fim_alert_email', 'api_token_v3', 'fim_excluded_paths', 'fim_scan_frequency',
        ...
        foreach ($text_fields as $field) {
            if (isset($input[$field])) {
                ...
                } elseif (strpos($field, 'urls') !== false || strpos($field, 'proxies') !== false || strpos($field, 'hashes') !== false || strpos($field, 'mentions') !== false || strpos($field, 'paths') !== false) {
        ...
        if (isset($new_input['api_token_v3']) && $new_input['api_token_v3'] !== ($this->plugin->options['api_token_v3'] ?? '')) {
            if (!empty($new_input['api_token_v3'])) {
                ...
                if (is_wp_error($response) || wp_remote_retrieve_response_code($response) !== 200) {
                    ...
                    $new_input['api_token_v3'] = $this->plugin->options['api_token_v3'] ?? '';
                }
            }
        }
    ...
    /**
     * Displays a toggle/switch field for settings.
     * Uses the HTML structure required to be styled with CSS.
     */
    ...
    {
        $name_attr = 'advaipbl_settings[log_timezone]';
        $current_timezone = $this->plugin->options['log_timezone'] ?? wp_timezone_string();
        echo '<select id="log_timezone" name="' . esc_attr($name_attr) . '">';
        echo wp_timezone_choice($current_timezone, get_user_locale());
        echo '</select>';
        echo '<p class="description">' . esc_html__('Select the timezone to display in the logs.', 'advanced-ip-blocker') . '</p>';
    }
    ...
    /**
    * Displays the description for the Threat Scoring section.
    */
    ...
    /**
     * Displays the table of numeric fields to configure points for each threat event.
     */
    ...
    public function restrict_login_page_callback()
    {
        ...
        if (! $is_whitelisted) {
            ...
            echo '<strong>' . sprintf(/* translators: %s is a placeholder */ esc_html__('Your current IP address (%s) is NOT whitelisted.', 'advanced-ip-blocker'), esc_html($client_ip)) . '</strong> ' . esc_html__('You MUST add it to the whitelist before enabling this feature, or you will be locked out.', 'advanced-ip-blocker');
            echo '<button class="button button-secondary advaipbl-add-whitelist-ajax" style="margin-left: 10px;" data-ip="' . esc_attr($client_ip) . '" data-detail="' . esc_attr__('Admin IP (self-added for login restriction)', 'advanced-ip-blocker') . '">' . esc_html__('Whitelist My IP Now', 'advanced-ip-blocker') . '</button>';
            echo '</p></div>';
        }
    }

    /**
     * Callback to show the V3 API connection status.
     */
    ...
    /**
     * Specific callback for the API Token field with obfuscation.
     */
    ...
    /**
 * Displays the <select> field for the XML-RPC protection mode.
 */
    public function xmlrpc_protection_mode_callback()
    {
        $current_mode = $this->plugin->options['xmlrpc_protection_mode'] ?? 'smart';
    ...
    /**
         * Displays the description for the Geoblocking section.
         */
    ...
    /**
     * Displays the multi-select field for countries, reusable for Geoblocking and Geo-Challenge.
     * @param array $args Arguments passed from add_settings_field.
     */
    public function geoblock_countries_callback($args)
    {
        $type = $args['type'] ?? 'geoblock';
    ...
    {
        $nonce_url = wp_nonce_url(
            admin_url('admin-post.php?action=advaipbl_clear_location_cache_action'),
    ...
    /**
     * Displays the selector for the geolocation method.
     */
    ...
    /**
     * Displays the status of the local GeoIP database.
     */
    ...
    /**
     * Displays the checkboxes to force 2FA by role.
     */
    ...
    <div id="advaipbl-recaptcha-v3-options-row">
        ...
        <p class="description"><?php  printf(/* translators: %s is a placeholder */ esc_html__('Logins with a score below this value (e.g., %s) will be blocked.', 'advanced-ip-blocker'), '0.5'); ?></p>
    </div>
    <?php
    }

    /**
     * Sanitizes and validates WAF rules before saving them.
     * - Removes duplicates.
     * - Discards invalid regular expressions.
     * - Discards empty or space-only rules.
     *
     * @param string $input The textarea content for WAF rules.
     * @return string The sanitized and validated content.
     */
    public function sanitize_waf_rules($input)
    {
        ...
        if ($invalid_rules > 0) {
            $message = sprintf(
                /* translators: %s is a placeholder */ _n(
    ...
    /**
         * Generic callback for action selectors (Block vs Challenge).
         * Accepts the field name dynamically.
         */
    ...
    /**
     * Displays the selection field for the AbuseIPDB action.
     */
    ...
    /**
         * Helper to generate the help icon if a URL exists.
         *
         * @param array $args Field arguments.
         * @return string Help link HTML or empty string.
         */
    ...
    public function display_captcha_keys_warning()
    {
        ...
        if (!empty($missing_keys_engines)) {
            ...
            echo sprintf(
                /* translators: %s is a placeholder */ esc_html__('You have selected %1$s as a challenge method but the API keys are missing. The system will automatically fall back to the JS Challenge to prevent locking out legitimate users. Please %2$sconfigure your keys%3$s.', 'advanced-ip-blocker'),
