...
class ADVAIPBL_FIM_Engine
{
    ...
    public function ajax_get_files()
    {
        ...
        $warning = '';
        $split_type = get_option('advaipbl_fim_signatures_split');
        $signatures_exist = ($split_type === '1' || $split_type === '2') ? true : !empty(get_option('advaipbl_fim_signatures'));
        ...
        $default_exclusions = [
            'wp-content/uploads/advaipbl-backups/',
            'wp-content/uploads/redux/',
            'wp-content/uploads/wpforms/cache/'
        ];
        $excluded_paths = array_unique(array_merge($excluded_paths, $default_exclusions));
    ...
    public function scan_for_malware($path)
    {
        ...
        if ($signatures === null) {
            $split_type = get_option('advaipbl_fim_signatures_split');
            if ($split_type === '2') {
                $signatures['raw'] = $this->get_chunked_option('advaipbl_fim_signatures_raw');
                $signatures['regex'] = $this->get_chunked_option('advaipbl_fim_signatures_regex');
                $signatures['domains'] = $this->get_chunked_option('advaipbl_fim_signatures_domains');
        ...
        if ($enable_md5 && !empty($md5_array)) {
            ...
            if (in_array($md5, $md5_array) || in_array($md5_prefix, $md5_array)) {
                $status = 'Found: Known Malware (MD5 Match)';
                $q_res = $this->internal_quarantine_file($path, 'MD5 Hash Match', true);
                if (!is_wp_error($q_res)) {
                    $status = 'Auto-Quarantined: Known Malware (MD5 Match)';
                }
                return $status;
            }
        }
        ...
        if ($enable_sha256 && !empty($sha256_array)) {
            ...
            if (in_array($sha256, $sha256_array)) {
                $status = 'Found: Known Malware (SHA256 Match)';
                $q_res = $this->internal_quarantine_file($path, 'SHA256 Hash Match', true);
                if (!is_wp_error($q_res)) {
                    $status = 'Auto-Quarantined: Known Malware (SHA256 Match)';
                }
                return $status;
            }
        }
        ...
        static $combined_regex = null;

        if ($enable_regex && !empty($signatures['regex'])) {
            if ($combined_regex === null) {
                $patterns = [];
                foreach ($signatures['regex'] as $chars) {
                    $pattern = $decode_obfuscated($chars);
                    $patterns[] = '(?:' . str_replace('~', '\~', $pattern) . ')';
                }
                $combined_regex = '~' . implode('|', $patterns) . '~is';
            }

            if (@preg_match($combined_regex, $content)) {
                $found[] = 'RegEx Match';
            }
        }
        ...
        $is_core_or_theme_file = in_array($basename, ['wp-config.php', 'wp-settings.php', 'wp-load.php', 'index.php', 'functions.php', 'wp-blog-header.php']);
        if ($is_core_or_theme_file) {
            $found = array_filter($found, function($item) {
                return strpos($item, 'AMW_Raw_') === false && $item !== 'Raw Signature';
            });
        }
    ...
    public function ajax_email_report()
    {
        ...
        $build_table = function ($title, $color, $icon, $items) {
            ...
            foreach ($items as $file) {
                ...
                if (is_array($file) && isset($file['status'])) {
                    if ($file['status'] === 'high_risk_scan') {
                        $status = __('High Risk File', 'advanced-ip-blocker');
                    } elseif ($file['status'] === 'uploads_scan') {
                        $status = __('Suspicious Upload', 'advanced-ip-blocker');
                    } elseif ($file['status'] === 'missing') {
                        $status = __('Missing File', 'advanced-ip-blocker');
                    } elseif ($file['status'] === 'modified') {
                        $status = __('Hash Mismatch', 'advanced-ip-blocker');
                    } elseif ($file['status'] === 'unrecognized') {
                        $status = __('Unrecognized File', 'advanced-ip-blocker');
                    }
                }
        ...
        $plugin_version = defined('ADVAIPBL_VERSION') ? ADVAIPBL_VERSION : '8.13.11';
...
// Silence is golden.");
    ...
    public function internal_quarantine_file($full_path, $malware_type, $is_auto = false)
    {
        $abs_path = wp_normalize_path(trailingslashit(ABSPATH));
        $file_path = str_replace($abs_path, '', wp_normalize_path($full_path));
        ...
        if (strpos($file_path, 'wp-admin/') === 0 || strpos($file_path, 'wp-includes/') === 0 || dirname($file_path) === '.') {
            return new WP_Error('core_file', 'Cannot quarantine WordPress core files.');
        }
        ...
        if (strpos($full_path, $active_theme_dir) === 0 || strpos($full_path, $parent_theme_dir) === 0) {
            return new WP_Error('theme_file', 'Cannot quarantine active theme files.');
        }

        if (strpos($file_path, 'wp-content/plugins/') === 0) {
            if (!$is_auto) {
                return new WP_Error('plugin_file', 'Cannot quarantine plugin files to prevent site breakage. Please disable the plugin and remove manually.');
            }
        }
        ...
        if (@rename($full_path, $vault_path)) { // phpcs:ignore WordPress.WP.AlternativeFunctions.rename_rename, PluginCheck.CodeAnalysis.WriteFile.ABSPATHDetected
            global $wpdb;
            ...
            if ($is_auto && isset($this->plugin->notification_manager)) {
                $this->plugin->notification_manager->send_auto_quarantine_notification($file_path, $malware_type);
            }

            return true;
        }

        return new WP_Error('move_failed', 'Failed to move file to quarantine vault.');
    }
    ...
    public function ajax_quarantine_file()
    {
        ...
        if (validate_file($file_path) !== 0) {
            wp_send_json_error('Path traversal detected. File must be strictly within the WordPress directory.');
        }

        $abs_path = wp_normalize_path(trailingslashit(ABSPATH));
        $full_path = wp_normalize_path($abs_path . $file_path);

        $result = $this->internal_quarantine_file($full_path, $malware_type, false);
    ...
    private function get_chunked_option($option_name) {
        $count = get_option($option_name . '_chunks', false);
        if ($count === false) {
            return get_option($option_name, []);
        }
        $result = [];
        for ($i = 0; $i < $count; $i++) {
            $chunk = get_option($option_name . '_' . $i, []);
            if (is_array($chunk)) {
