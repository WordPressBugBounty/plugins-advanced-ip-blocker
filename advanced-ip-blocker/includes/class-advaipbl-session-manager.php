...
class ADVAIPBL_User_Session_Manager
{
    ...
    <div class="tablenav top">
        ...
        <div class="tablenav-pages">
            <span class="displaying-num">
                <?php  printf(esc_html(/* translators: %s is a placeholder */ _n('%s session', '%s sessions', $total_items, 'advanced-ip-blocker')), esc_html(number_format_i18n($total_items))); ?>
            </span>
            <?php
            ...
            <?php else : foreach ($sessions_page as $session) :
                ...
                if ($location) {
                    ...
                    if (! empty($location['country'])) {
                        $location_parts[] = $location['country'] . (!empty($location['country_code']) ? ' (' . $location['country_code'] . ')' : '');
                    }
                } ?>
                <tr>
    ...
    <div style="margin-top:20px;">
        ...
        <select id="role-selector"><option value=""><?php esc_html_e('-- Select a Role --', 'advanced-ip-blocker'); ?></option>
            <?php foreach (array_keys($all_roles) as $role) : if (empty($role)) {
                continue;
            } ?>
    ...
    public function ajax_close_user_session()
    {
        ...
        if ($user_id > 0) {
            ...
            if ($user_to_logout) {
                $message = sprintf(/* translators: %s is a placeholder */ __('Sessions for user "%1$s" were closed by %2$s.', 'advanced-ip-blocker'), $user_to_logout->user_login, wp_get_current_user()->user_login);
                if (isset($this->plugin_instance)) {
                    $this->plugin_instance->log_event($message, 'warning');
                }
            }
    ...
    public function ajax_close_all_user_sessions()
    {
        ...
        $message = sprintf(/* translators: %s is a placeholder */ __('All sessions for ALL users were closed by %s.', 'advanced-ip-blocker'), wp_get_current_user()->user_login);
        if (isset($this->plugin_instance)) {
            $this->plugin_instance->log_event($message, 'critical');
        }
        wp_send_json_success();
    }
    ...
    public function ajax_close_sessions_by_role()
    {
        ...
        if (!empty($role)) {
            ...
            $message = sprintf(/* translators: %s is a placeholder */ __('All sessions for role "%1$s" were closed by %2$s.', 'advanced-ip-blocker'), $role, wp_get_current_user()->user_login);
            if (isset($this->plugin_instance)) {
                $this->plugin_instance->log_event($message, 'warning');
            }
        }
    ...
    {
        if (!current_user_can('advaipbl_manage_settings') || !check_admin_referer('advaipbl_usm_save_settings_nonce')) {
            wp_die(esc_html__('Action not allowed.', 'advanced-ip-blocker'));
        }$per_page = isset($_POST['sessions_per_page']) ? intval(wp_unslash($_POST['sessions_per_page'])) : 10;
        update_option('advaipbl_usm_sessions_per_page', max(1, min(100, $per_page)));
        wp_safe_redirect(admin_url('admin.php?page=advaipbl_settings_page&tab=dashboard&sub-tab=user_sessions'));
        exit;
    }
    ...
    /**
     * Gets locations for an array of IPs, using our custom cache table.
     *
     * @param array $ips An array of IPs to locate.
     * @return array An associative array of [ip => location_data].
     */
    ...
    public function parse_user_agent($ua)
    {
        if (empty($ua) || $ua === __('Unknown', 'advanced-ip-blocker')) {
            return __('Unknown', 'advanced-ip-blocker');
        }
        $os = __('Unknown OS', 'advanced-ip-blocker');
        $browser = __('Unknown Browser', 'advanced-ip-blocker');
        if (preg_match('/(Windows|Macintosh|Android|iPhone|iPad|Linux)/i', $ua, $matches)) {
            $os = $matches[1];
        }
        ...
        return sprintf(/* translators: %s is a placeholder */ __('%1$s on %2$s', 'advanced-ip-blocker'), $browser, $os);
    }
}
