...
class ADVAIPBL_Notification_Manager
{
    ...
    private function send_grouped_summary_email($notifications, $to, $type)
    {
        ...
        $button_url = admin_url('admin.php?page=advaipbl_settings_page&tab=ip_management&sub-tab=blocked_ips');
        if ($type === 'signature') {
    ...
    /**
     * Sends Auto Quarantine Notification (Email + Push).
     */
    public function send_auto_quarantine_notification($file_path, $malware_type)
    {
        $options = $this->plugin->options;
        $site_name = get_bloginfo('name');
        ...
        if (!empty($options['enable_push_notifications'])) {
            $message_lines = [
                sprintf('*:warning: [%s] Malware Auto-Quarantined*', $site_name),
                '> The File Integrity Monitor has automatically quarantined a malicious file.',
                sprintf('>   *File Path:* `%s`', $file_path),
                sprintf('>   *Malware Signature:* `%s`', $malware_type),
            ];
            $this->execute_webhook_send(implode("\n", $message_lines));
        }

        if (!empty($options['enable_email_notifications'])) {
            // Check if alert email is specifically configured for FIM
            $to = !empty($options['fim_alert_email']) ? $options['fim_alert_email'] : (!empty($options['notification_email']) ? $options['notification_email'] : get_option('admin_email'));

            /* translators: %s is a placeholder */
            $email_subject = sprintf(__('[%1$s] Security Alert: Malware Auto-Quarantined', 'advanced-ip-blocker'), $site_name);
            $template_title = __('Malware Auto-Quarantined', 'advanced-ip-blocker');
            $button_url = admin_url('admin.php?page=advaipbl_settings_page&tab=integrity&sub-tab=fim_dashboard');

            $content_html =
                '<p style="font-size: 16px; line-height: 1.6;">' . esc_html__("The File Integrity Monitor has detected an exact malware signature match (MD5 or SHA256) and has automatically moved the malicious file to the quarantine vault to protect your site.", 'advanced-ip-blocker') . '</p>' .
                '<table style="width: 100%; border-collapse: collapse; margin-top: 20px;">' .
                '<tr><td style="padding: 8px; border: 1px solid #ddd; background-color: #f9f9f9; width: 150px;"><strong>' . esc_html__('Quarantined File', 'advanced-ip-blocker') . '</strong></td><td style="padding: 8px; border: 1px solid #ddd;"><code>' . esc_html($file_path) . '</code></td></tr>' .
                '<tr><td style="padding: 8px; border: 1px solid #ddd; background-color: #f9f9f9;"><strong>' . esc_html__('Malware Signature', 'advanced-ip-blocker') . '</strong></td><td style="padding: 8px; border: 1px solid #ddd;"><code>' . esc_html($malware_type) . '</code></td></tr>' .
                '</table>' .
                '<p style="margin-top: 20px; font-size: 14px; color: #555;">' . esc_html__('The file is safely disabled. You can review and delete it from the Quarantine Vault in the plugin dashboard.', 'advanced-ip-blocker') . '</p>'.
            '<table style="width: 100%; text-align: center; margin-top: 30px;"><tr><td>' .
            '<a href="' . esc_url($button_url) . '" style="background-color: #d63638; color: #ffffff; padding: 12px 25px; text-decoration: none; border-radius: 4px; display: inline-block; font-weight: bold;">' . esc_html__('View Quarantine Vault', 'advanced-ip-blocker') . '</a>' .
            '</td></tr></table>';
        ...
        $body = $this->get_html_email_template($template_title, $content_html, $fim_settings_url);
        add_filter('wp_mail_content_type', [$this, 'set_html_mail_content_type']);
            wp_mail($to, $email_subject, $body);
            remove_filter('wp_mail_content_type', [$this, 'set_html_mail_content_type']);
        }
     ...
     */
    public function get_html_email_template($title, $content, $custom_settings_url = null)
    {
            ...
            <div class="email-container">
                ...
                <div class="email-footer">
                    ...
                    <p style="margin: 5px 0 0;">
                        <?php
                        $settings_url = $custom_settings_url ? $custom_settings_url : admin_url('admin.php?page=advaipbl_settings_page&tab=settings&sub-tab=general_settings');
