...
class ADVAIPBL_Api_Manager
{
    ...
    private function fetch_from_ipapi($ip)
    {
        ...
        if (empty($api_key)) {
            ...
            return ['error' => true, 'error_message' => sprintf(__('API Key is required for %s', 'advanced-ip-blocker'), 'ipapi.com')];
        }
    ...
    private function fetch_from_ipstack($ip)
    {
        ...
        if (empty($api_key)) {
            /* translators: %s is a placeholder */
            return ['error' => true, 'error_message' => sprintf(__('API Key is required for %s', 'advanced-ip-blocker'), 'ipstack.com')];
        }
