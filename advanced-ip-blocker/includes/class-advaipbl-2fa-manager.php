...
{
    public const META_SECRET           = '_advaipbl_2fa_secret';
    public const META_ENABLED_AT       = '_advaipbl_2fa_enabled_at';
    public const META_BACKUP_CODES     = '_advaipbl_2fa_backup_codes';
    public const META_TEMP_SECRET      = '_advaipbl_2fa_temp_secret';
    ...
    public function is_2fa_enabled_for_user(int $user_id): bool
    {
        return (bool) get_user_meta($user_id, self::META_ENABLED_AT, true);
    }
    ...
    {
        if ($this->is_valid_backup_code($user_id, $code)) {
            return true;
...
return $this->tfa->verifyCode($secret, $code, 1);
    ...
    public function is_2fa_forced_for_user(WP_User $user): bool
    {
        $forced_roles = $this->main_class->options['tfa_force_roles'] ?? [];
        if (empty($forced_roles)) {
            return false;
        }

return ! empty(array_intersect((array) $user->roles, $forced_roles));
    }

    public function generate_backup_codes(int $count = 8, int $length = 10): array
    {
        $codes = [];
        for ($i = 0; $i < $count; $i++) {
            $code = wp_generate_password($length, false);
            if ($length > 5) {
                $code = substr($code, 0, $length / 2) . '-' . substr($code, $length / 2);
            } $codes[] = $code;
        }
...
return $codes;
    ...
    public function hash_backup_codes(array $codes): array
    {
        $hashed_codes = [];
